...
class EnrollmentController {
  async index(req, res) {
    try {
      ...
      console.log(error);
      return res.status(400).json({ error: 'Error on trying to find the enrollments.' });      
    }
  ...
  async store(req, res) {
    try {
      ...
      const findEnrollment = await Enrollment.findOne({ where: { student_id, 
                                                                end_date: { 
                                                                  [Op.gte] : new Date()
                                                                } 
                                                              }
      ...
      if (findEnrollment) {
        return res.status(400).json({ error: 'There is already a valid enrollment for the student informed.' });
      }
      ...
      console.log(error);
      return res.status(400).json({ error: 'Error on trying to create the enrollment.' });
    }
  }
  ...
  async update(req, res) {
    try {
      if (Object.keys(req.body).length === 0)
        ...
        if (isBefore(start_date_formated, new Date())) {
          return res.status(400).json({ error: `Past dates are not permitted: ${start_date_formated}.` });
        }
        
        let end_date = findEnroll.end_date;
      ...
      console.log(error);
      return res.status(400).json({ error: 'Error on trying to update the enrollment.' });      
    }
  }
